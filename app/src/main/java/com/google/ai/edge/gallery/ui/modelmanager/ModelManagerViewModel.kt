...
constructor(
  ...
  // Customized loadModelAllowlist to only load Gemma-3n-E2B-it-int4
    ...
    viewModelScope.launch(Dispatchers.IO) {
      try {
        ...
        if (modelAllowlist == null) {
          Log.d(TAG, "Failed to load model allowlist from internet. Trying disk...")
          modelAllowlist = readModelAllowlistFromDisk()
        } else {
          Log.d(TAG, "Model allowlist loaded from internet.")
        ...
        val selectedModelName = "Gemma-3n-E4B-it-int4"
        val allowedModel = modelAllowlist.models.find { it.name == selectedModelName }

        if (allowedModel == null || allowedModel.disabled == true) {
          Log.d(TAG, "Model '$selectedModelName' not found or disabled in allowlist")
          _uiState.update {
            it.copy(
              loadingModelAllowlist = false,
              loadingModelAllowlistError = "Model not found or disabled"
            )
          }
          return@launch
        }
        ...
        val modelInstances = mutableMapOf<String, ModelInitializationStatus>()
        ...
        Log.d(TAG, "Only model '${model.name}' loaded successfully.")
        ...
        _uiState.update {
          it.copy(
            loadingModelAllowlist = false,
            loadingModelAllowlistError = e.message ?: "Unexpected error"
          )
        }
      }
    }
